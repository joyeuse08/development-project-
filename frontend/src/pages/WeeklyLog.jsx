    ...
    >
      ...
      <p style={styles.date}>
        Submitted: {log.submitted_at
          ? new Date(log.submitted_at).toLocaleDateString("en-GB")
          : "Not yet"}
      </p>
    </div>
  );
  ...
  return (
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <span style={{ ...styles.badge, color: status.color, background: status.bg, marginBottom: 12, display: "inline-block" }}>
          {status.label}
        </span>
        ...
        {[
          { label: "Activities", value: log.activities },
          { label: "Challenges", value: log.challenges },
          { label: "Learnings", value: log.learnings },
          { label: "Feedback", value: log.feedback },
        ].map(({ label, value }) => (
          <div key={label} style={styles.detailSection}>
            ...
            <p style={{ margin: "6px 0 0", color: "#444", lineHeight: 1.7, fontSize: 14 }}>
              {value || "—"}
            </p>
          </div>
        ))}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 4 }}>
          ...
          <div style={styles.detailSection}>
            ...
            <p style={{ margin: "6px 0 0", color: "#444", fontSize: 13 }}>
              {log.submitted_at ? new Date(log.submitted_at).toLocaleString() : "Not yet"}
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{ ...styles.card, marginBottom: 28 }}>
      ...
      {[
        ...
        { name: "learnings", label: "Learnings" },
      ].map(({ name, label }) => (
      ...
      <div style={styles.page}>
        ...
        {!loading && !error && logs.length === 0 && (
          <div style={styles.emptyBox}>
            ...
            <p>No weekly logs yet. Submit your first log!</p>
          </div>
        )}
        {!loading && !error && logs.length > 0 && (
          <div style={styles.grid}>
            {logs.map((log) => <LogCard key={log.id} log={log} onView={setSelectedLog} />)}
          </div>
        )}
      </div>
