  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <h3 style={styles.cardTitle}>Week {feedback.weekly_log_week || "—"}</h3>
...
function SubmitFeedbackForm({ onSuccess }) {
  const [form, setForm] = useState({ weekly_log: "", comments: "", supervisor_score: "" });
  ...
  setMessage(null);
  try {
    await api.post('/api/Supervisor_Feedback/', form);
    setMessage({ type: "success", text: "Feedback submitted successfully!" });
    setForm({ weekly_log: "", comments: "", supervisor_score: "" });
    onSuccess();
  } catch (err) {
    ...
    <div style={{ ...styles.card, marginBottom: 28 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
        <div style={styles.formGroup}>
          <label style={styles.formLabel}>Weekly Log ID</label>
          <input name="weekly_log" type="number" value={form.weekly_log} onChange={handleChange} style={styles.input} placeholder="Log ID" />
        </div>
        <div style={styles.formGroup}>
          <label style={styles.formLabel}>Score (0-100)</label>
          <input name="supervisor_score" type="number" value={form.supervisor_score} onChange={handleChange} style={styles.input} placeholder="e.g. 85" min="0" max="100" />
        </div>
      </div>
...
export default function SupervisorFeedback() {
  ...
  api.get('/api/Supervisor_Feedback/')
    .then((res) => {
...
const styles = {
  ...
  scoreBadge: { fontSize: 13, fontWeight: 700, color: "#f39c12", background: "#fef9ec", padding: "3px 12px", borderRadius: 20 },
