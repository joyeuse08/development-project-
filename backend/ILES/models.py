...
class Weekly_Log(models.Model):
    ...
    placement = models.ForeignKey(
        Internship_Placement, on_delete=models.CASCADE, related_name='weekly_logs'
    )
...
class Supervisor_Feedback(models.Model):
    weekly_log = models.OneToOneField(
        Weekly_Log, on_delete=models.CASCADE, related_name='supervisor_feedback'
    )
...
class Academic_Supervisor_Feedback(models.Model):
    placement = models.ForeignKey(
        Internship_Placement, on_delete=models.CASCADE, related_name='academic_feedbacks'
    )
    academic_supervisor = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE,
        related_name='academic_feedback_given',
        limit_choices_to={'role': 'academic'},
    )
    comments = models.TextField()
...
class Weighted_Score(models.Model):
    placement = models.OneToOneField(
        Internship_Placement, on_delete=models.CASCADE, related_name='weighted_scores'
    )
...
class Issue(models.Model):
    ...
    placement = models.ForeignKey(
        Internship_Placement, on_delete=models.CASCADE, related_name='issues'
    )
    ...
    def __str__(self):
        return f"Issue '{self.title}' for {self.placement.student.username}"
